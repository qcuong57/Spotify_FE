...
// --- CÁC VARIANTS MỚI CHO THÔNG BÁO THÀNH CÔNG ---
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res && res.data) {
        ...
        setCurrentView(updatedPlaylist); // Cập nhật view MyLibrary (ĐÚNG)
        // alert("Cập nhật playlist thành công!"); // <-- BỎ DÒNG NÀY
        
        setIsSuccess(true); // <-- THÊM DÒNG NÀY
        setLoading(false); // <-- DI CHUYỂN LÊN ĐÂY

        setTimeout(() => { // <-- THÊM ĐỂ TỰ ĐỘNG ĐÓNG
          onClose();
      ...
      setLoading(false); // <-- Giữ lại ở đây để xử lý lỗi
    } 
    // BỎ KHỐI finally
  };
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* --- THÊM KHỐI AnimatePresence VÀ ĐIỀU KIỆN isSuccess --- */}
          <AnimatePresence mode="wait">
            {isSuccess ? (
              <motion.div
                key="success"
                className="flex flex-col items-center justify-center p-10 space-y-4 min-h-[300px]" // Thêm chiều cao
